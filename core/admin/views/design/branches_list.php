...
<? if ($branch->id != 1 && $can_push): ?>
					<a href="<?= $this->urlTo("/design/branches/push/{$id}") ?>"><img title="push branch" alt="push branch" src="<?= $image_root.'down-arrow.png' ?>" /></a>
<? endif; ?>
<? if ($branch->id != 3 && $can_rollback): ?>
<? $rb = $id + 1; ?>
					<a href="<?= $this->urlTo("/design/branches/rollback/{$rb}") ?>"><img title="rollback branch" alt="rollback branch" src="<?= $image_root.'up-arrow.png' ?>" /></a>
<? endif; ?>
				</div>
			</div>
		</li>
